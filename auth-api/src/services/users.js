...
const AUTH_DATA_DIR = process.env.AUTH_DATA_DIR || '/ssd_pool/auth-service/data';
const USERS_FILE = path.join(AUTH_DATA_DIR, 'users.yml');
...
const DEFAULT_GROUPS = {
  ...
  power_users: {
    ...
    description: 'Accès étendu avec 2FA',
    policy: 'two_factor'
  },
  users: {
    displayName: 'Utilisateurs',
    description: 'Accès basique avec 1FA',
    policy: 'one_factor'
  }
};
...
export async function createUser(username, password, displayname, email, groups = ['users']) {
  ...
  if (!/^[a-z0-9_-]{3,32}$/i.test(username)) {
    return { success: false, error: 'Nom d\'utilisateur invalide (3-32 caractères, lettres, chiffres, _ ou -)' };
  }
...
// Migrate users from Authelia format if needed
export async function migrateFromAuthelia(autheliaUsersPath) {
  try {
    if (!fs.existsSync(autheliaUsersPath)) {
      return { success: false, error: 'Fichier Authelia non trouvé' };
    }

    const autheliaData = yaml.load(fs.readFileSync(autheliaUsersPath, 'utf8'));
    const currentData = loadUsers();

    let migrated = 0;
    for (const [username, userData] of Object.entries(autheliaData.users || {})) {
      if (!currentData.users?.[username]) {
